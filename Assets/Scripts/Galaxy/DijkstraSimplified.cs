...
//Largely unedited from the template given, but added support for capping the maximum distance of between stars and total.
//Edited lines are 12-14, 22-25, 42-43, 51-53, 68
...
public static class DijkstraSimplified {
	...
	//Also need to have a custom class to hold them.
	private static List<Star> galaxyStarList;
	//New attributes added from CanvasScript
	public static float maxTotalDistance;
	public static float maxJumpDistance;
	...
	public static List<Star> FindPath(Star startStar, Star endGoalStar) {
		...
		//If the maximum total distance is unset, set to infinity (functions the same as the original code).
		if (maxTotalDistance == 0) {
			maxTotalDistance = float.MaxValue;
		}
		...
		//Go through the priority list and for each star look at its connections and find the distance to the start star.
		...
		//End the loop if the shortest distance to start exceeds the maximum.
		while (priorityList.Count > 0 && priorityList[0].smallestDistanceToStart < maxTotalDistance) {
			...
			foreach (StarPath nextStar in GetStarPathsFromRoutesDictionary(currentStarNode.star.starRoutes, priorityList)) {
				...
				if (nextStar.star.starRoutes[currentStarNode.star] > maxJumpDistance && maxJumpDistance != 0) {
			...
			if (priorityList.Count > 0) {
				...
				if (priorityList[0].star == endGoalStar && priorityList[0].smallestDistanceToStart < maxTotalDistance) {
					//If the star with the highest priority is now the end star then that means we have found the shortest path to the end.
					//However the distance to the start must not be greater than the maximum (formerly infinity) else that just means we haven't found a connection.
